...
As we can see from the summary, about 52% of the respondents are femail and 48% male. The mean age of the participants is 53,5 years and about 71% of them have children. The average duration of marriage among the participants has been about 8 years.  


```{r echo=TRUE, error=FALSE, warning=FALSE, message=FALSE}
# printing a graphical summarization of affairs
gather(affairs) %>% ggplot(aes(value,fill="#99999",alpha=0.3)) + facet_wrap("key", scales = "free") + geom_bar() + ggtitle("Barsplot of all variables") +
  theme(plot.title = element_text(hjust = 0.5,size=12,face='bold'),legend.position="none")
```

When we look at a bar visualization of the data, we can see that most participants have had at least some years of college education, have had no extra-marital relationships, are in administrative or managerial positions, are happy with their marriage, and are somewhat religious.  
...
When we cross-tabulate the variables, we can see a negative correlation between the number of extra-marital affairs and how people rate their marriage. There's also some correlation between extra-marital affairs and years of marriage and negative correlation with religiousness. Next, we look at the relation of these correlating pairs more closely.
